...
try {
    if ($statement = $database_handler->prepare("UPDATE memos SET title = :title,content = :content,updated_at = NOW() WHERE id =:edit_id AND user_id = :user_id")) {
        $edit_title_specialchars = htmlspecialchars($edit_title);
        $edit_content_specialchars = htmlspecialchars($edit_content);
        $statement->bindParam(":title", $edit_title_specialchars);
        $statement->bindParam(":content", $edit_content_specialchars);
